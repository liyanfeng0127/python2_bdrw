...
def towrite(parm):
    ...
    # f.writelines(u'职位月薪：' + str(parm['recruit_salary']) + '\n')
...
def spider(url):
    #jl=&kw=&sm=0&isfilter=1&fl=530&isadv=0&sb=1&sg=8de28363eb064a2b8ab40da3111bbceb&p=
    param = {
        'bj': 160000,
        'kw': '嵌入式',
        'sm': 0,
        ...
        'isadv': 0,
        'sb': 1,
        'sg': '8de28363eb064a2b8ab40da3111bbceb',
        'p': 2,
    }
    html = requests.get(url,params= param).content
    selector = etree.HTML(html)
    print selector
    recruit = selector.xpath('//div[@id="newlist_list_content_table"]/table[@class="newlist"]')
    #print recruit
    item = {}
    for each in recruit:
        # recruit_jobs = each.xpath('/tbody/tr/td[@class="zwmc"]/div/a')
        ...
        # print recruit_company_net
        recruit_company = each.xpath('./tbody/tr[1]/td[@class="gsmc"]/a/text()')
        print recruit_company
...
if __name__ == '__main__':
    ...
    # pool = ThreadPool(4)
    # f = open('Embedded_recruitment.csv','wt')
    # page = []
    # for i in range(1, 5):
    #     i=1
    ...
    # pool.join()
    # f.close()
    page = 'http://sou.zhaopin.com/jobs/searchresult.ashx?'
    result = spider(page)